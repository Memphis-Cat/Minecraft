...
public:
    ...
    void Render(const World& world, DirectX::XMFLOAT3 cameraPosition, DirectX::CXMMATRIX viewProjection, int renderDistanceChunks);
private:
    struct GpuChunk { Microsoft::WRL::ComPtr<ID3D11Buffer> vertexBuffer; UINT vertexCount{}; std::uint64_t revision{}; };
    bool CreateBackBuffer(int width, int height);
    bool CreatePipeline();
    bool CreateAtlas();
    bool IsChunkVisible(ChunkCoord coord, DirectX::CXMMATRIX viewProjection) const;
